...
```
   ...
   **Never show internal machinery.** Detail identifiers (`training_default`, `subprocessors`, …), match scores, tool names, and "which detail answered which questions" tables are debugging output, not a customer experience. Refer to a detail by what it says, never by its id. The user sees questions, suggested answers, and sources — nothing about how the lookup works.
   ...
   2-3. **Walk the questionnaire ONE CARD PER QUESTION. No lists, no summaries, no section digests.** This is the whole experience: the user moves through their own form, one question at a time, with an answer already filled in for them.
      ...
      **When the profile has no answer** — the same card shape, with the answer line reading `No answer in your materials` — then ask the user for it rather than moving on silently. Options: `Type the answer` (their free text becomes the answer, recorded with them as its source), **`Skip for now`**, `Skip all remaining`. One question, one card — never collect gaps into a list and never explain why the answer is missing.
      **`Skip for now` is the important one, and it is the second option deliberately** (Roee, 2026-08-26): an in-house counsel may need two days to answer a single question, and the flow must never block on that. Skipped is not abandoned — it means "come back to it". In the export, a skipped question is marked `to answer` rather than left silently empty, so the user can see at a glance what is still owed.
