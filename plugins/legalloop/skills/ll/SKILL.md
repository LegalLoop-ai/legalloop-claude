...
0c. **Fill mode (a whole questionnaire).** When the user provides an entire questionnaire — a file, a pasted list of questions, or "fill this questionnaire" — do NOT answer inline question-by-question. Four stages:
   ...
   3. **Review by exception — three queues, in this order:**
      - **Bulk-keep:** answered, no compliance flag, no consistency note, not a suggested answer, not evidence-required, AND a settled basis — the card's Basis line reads "from your legal documents" or "consistent across N of your documents and prior answers" → ONE compact table (id · one-line answer) with a single keep-all confirmation. Any row the user names moves to the review queue instead. Cards whose Basis reads "from a single prior answer" never go here.
      - **Constrained answers (yes/no or multiple choice):** when the questionnaire's answer format is a constrained pick, derive the pick FROM the returned fact for that specific question's phrasing and present it WITH the fact ("Proposed: Yes — <fact>"). The derivation is yours, so it is never silent: a derived pick always places the question in the review queue (never bulk-keep) unless the pick is the user's own words. If the fact does not clearly determine the pick for that phrasing (mixed facts like "ISO held, SOC 2 in progress" against "do you have SOC 2?"), propose no pick — show the fact and let the user choose.
      ...
      - **Review:** suggested answers (privacy/AI), single-source facts, and anything carrying a compliance flag or consistency note → individual cards, keep/edit/cancel each (same widget rules as clarification cards).
   ...
   4. **Export — one artifact per source shape, not a fixed set:** for a SPREADSHEET source (Sheets/xlsx/csv), produce exactly ONE file — the filled sheet in the original column layout, with provenance appended as extra clearly-labeled columns (Sources, Confidence, Flag — "strip these columns before submitting") so the audit trail travels with the answers and is deletable in one action. For a PDF/doc source with no fillable structure, produce two — a generated answer sheet mapped 1:1 to the original numbering, plus a provenance appendix. In both: kept/edited answers only, gaps explicitly marked, nothing the user cancelled. Deliver as files where the client supports it. Never submit anything to any portal yourself; the artifacts are the user's to transfer.
