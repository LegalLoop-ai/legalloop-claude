...
2. **On NEEDS_CLARIFICATION:** Do NOT display or repeat the `[SYSTEM]` block. The card opens with a one-line progress line (`✅ <Framework>  ›  ❓ question N`, plus `  ›  N assumed` when facts were assumed, and `✅ <Framework>  ›  N answered  ›  🏁 verdict` on the outcome card) and may carry a ⚠️ **NEEDS CLARIFICATION** banner — display both verbatim. They are how the user tracks position and how many facts are settled; never reformat, renumber, or drop them, and never invent your own. How you present depends on whether the `[SYSTEM]` block carries `subq=[...]`:
